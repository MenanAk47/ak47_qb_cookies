    ...
    window.addEventListener('message', function(event) {
        if (event.data.type == 'shop') {
            ...
            $('.image').on('click', function() {
                ...
                $.post('http://ak47_qb_cookies/putcart', JSON.stringify({
                    item: item,
                    price: price,
                    label: label,
                    count: count,
            ...
            $("body").on("click", "#refreshcart", function() {
                $.post('http://ak47_qb_cookies/escape', JSON.stringify({}));
                location.reload(true);
                $('#wrapper').hide("fast");
                $('#payment').hide("fast");
                $('#cart').hide("fast");
                $.post('http://ak47_qb_cookies/refresh', JSON.stringify({}));
            });
            ...
            $("body").on("click", "#buybutton", function() {
                ...
                for (i = 0; i < value.length; i++) {
                    ...
                    if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
                        ...
                        $('#payment').hide("fast");
                        $('#cart').hide("fast");
                        $.post('http://ak47_qb_cookies/buy', JSON.stringify({
                        	Price: price,
                            Count: value[i].value,
                            Item: value[i].id
                        }));
                    } else {
                        $.post('http://ak47_qb_cookies/notify', JSON.stringify({
            ...
            $("body").on("click", "#bossactions", function() {
                $.post('http://ak47_qb_cookies/bossactions', JSON.stringify({}));
                $.post('http://ak47_qb_cookies/escape', JSON.stringify({}));
                location.reload(true);
                $.post('http://ak47_qb_cookies/emptycart', JSON.stringify({}));
                $('#wrapper').hide("fast");
                $('#payment').hide("fast");
                $('#cart').hide("fast");
            });
